...
def change_timezone(parsed_date_data: Union[ParsedDate, Any],
    ...
    origin_date = \
        datetime.datetime(year=parsed_date_data.year, month=parsed_date_data.month,
                          day=parsed_date_data.day, hour=parsed_date_data.hour,
                          minute=parsed_date_data.minute, second=parsed_date_data.second)
...
def transform_hms(result_str: Union[str, Any],
                   hms_token: Union[str, Any],
                   ispm: Union[bool, Any],
                   hms_value: Union[int, Any]) -> Any:
    """
    This function transform parsed month into target format
    Parameters
    ----------
    ...
    if not timezone_token is None:
        if timezone_token == 'z':
            ...
            result = result.replace(timezone_token,
                                    "UTC" + utc_add + offset_hours_str + ":" + offset_minutes_str)
    return result
