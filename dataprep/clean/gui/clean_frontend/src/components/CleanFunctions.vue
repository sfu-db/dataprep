<template>
    <el-row>
      ...
      <el-col :span="3" class="el-col-select-box0">
          <el-select v-model="initColValue" placeholder="Select Col" @change="changeColSelection">
                  <el-option
      ...
      <el-col :span="3" class="el-col-select-box" v-for="(item, key) in paramDic">
        <div class="titles"><p>{{key}}</p></div>
        <el-select v-model="paramDefault[key]" @change="changeParamSelection(key, $event)">
          <el-option
      ...
      <el-col :span="3"  >
        <el-button  icon="el-icon-circle-check" class="el-col-select-box-button" @click="clickButton"><br>OK!</el-button>
      </el-col>
    </el-row>
</template>
<script>
...
export default ({
  name:'CleanEmail',
  props:['filenname', 'clean_func', 'table_cols', 'param_dic', 'param_default'],
  data(){
    return{
      cleanFunction: '',
      tableCols: [],
  ...
  methods:{
    ...
    clickButton:function(){
      ...
      console.log(JSON.stringify(_this.selectedParams))
      ...
      axios
        .post("http://localhost:7680/cleanSingleCol", param, config)
        .then(function(res){
          console.log(res)
