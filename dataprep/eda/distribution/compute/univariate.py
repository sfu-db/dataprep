...
def cont_comps(srs: dd.Series, bins: int) -> Dict[str, Any]:
    ...
    # In try-except:
    # when the sample size is less than 8, There is valueError caused by normaltest(),
    # In that case, we will use the default bins generated by da.histogram
    try:
        frmtd_bins = get_intervals(data["min"], data["max"], bins)
        data["hist"] = da.histogram(srs, bins=frmtd_bins)
    ...
    data["dens"] = da.histogram(srs, bins=frmtd_bins, density=True)
...
def calc_cat_stats(
    ...
    hist = da.histogram(lengths.values, bins=frmtd_bins)
    leng = {
        "Mean": lengths.mean(),
