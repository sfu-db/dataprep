...
def test_normal_1() -> None:
    ...
    df_1_expected: Dict[str, Dict[str, Union[Dict[Any, Any], Tuple[Any,
                                                                   Any]]]] = \
                    {"bool_01": {"bar_plot": {0: 4, 1: 5}},
                     ...
                     "s1": {"bar_plot": {1.0: 9}},
                     "x": {"histogram": (np.array([1, 3, 1, 0, 1, 0, 0, 0, 0,
                                                   2], dtype=np.int64),
                                         np.array([-10., -4., 2., 8., 14.,
                                                   20., 26.,
                                                   32., 38., 44., 50.]))},
                     'y': {'histogram': (np.array([6, 0, 1, 0, 0, 0, 0, 0, 0,
                                                   1], dtype=np.int64),
                                         np.array([-3.14159265, 309.37256661,
                                                   621.88672588,
                                                   934.40088514,
                                                   1246.91504441, 1559.42920367,
                                                   1871.94336294,
                                                   2184.4575222, 2496.97168147,
                                                   2809.48584073, 3122.]))}
                    }
    ...
    for intermediate in returned_1:
        ...
        if  field in df_1_expected:
            ...
            if "histogram" in result:
                assert np.allclose(result["histogram"][0],
                                   df_1_expected[field]["histogram"][0],
                                   equal_nan=True)
                assert np.allclose(result["histogram"][1],
    ...
    for intermediate in returned_2:
        ...
        if "box_plot" in result:
            ...
            assert np.allclose(result["histogram"]["a"][0],
                               df_2_expected["histogram"]["a"][0],
                               equal_nan=True)
            assert np.allclose(result["histogram"]["b"][0],
                               df_2_expected["histogram"]["b"][0],
                               equal_nan=True)
            assert np.allclose(result["histogram"]["c"][0],
                               df_2_expected["histogram"]["c"][0],
                               equal_nan=True)
            assert np.allclose(result["histogram"]["d"][0],
    ...
    df_2_expected_2 = {"stacked_column_plot": {("a", Timestamp("1898-01-02 00:00:00")): 1,
