...
def test_plot_corr_df() -> None:  # pylint: disable=too-many-locals
    ...
    start_p = time()
    _, intermediate = plot_correlation(pd_data_frame=df_data, return_intermediate=True)
    end_p = time()
...
def test_plot_corr_df_x_k() -> None:
    ...
    _, _ = plot_correlation(
        pd_data_frame=df_data, x_name=x_name, return_intermediate=True, k=k
    )
...
def test_plot_corr_df_x_y_k() -> None:
    ...
    _, intermediate = plot_correlation(
        pd_data_frame=df_data_cat, x_name="b", y_name="c", return_intermediate=True
    )
    assert np.isclose(
