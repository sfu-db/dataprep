...
def _calc_none_sum(series: pd.Series, length: int) -> Any:
    """
...
def test_plot_missing_df() -> None:
    ...
    for column_name in column_name_list:
        count_none_list.append(dask.delayed(_calc_none_sum)(df_data[column_name], 100))
...
def test_plot_missing_df_x() -> None:
    ...
    _, _ = plot_missing(pd_data_frame=df_data, x_name="a", return_intermediate=True)
    df_data = pd.DataFrame({"a": [random.choice(numbers_nan) for _ in range(100)]})
    df_data["b"] = [random.choice(numbers) for _ in range(100)]
...
def test_plot_missing_df_x_y() -> None:
    ...
    df_data["d"] = [random.choice(numbers) for _ in range(100)]
    _, _ = plot_missing(
        pd_data_frame=df_data, x_name="a", y_name="b", return_intermediate=True
    )
    df_data["c"] = df_data["c"].astype("category")
    _, _ = plot_missing(
        pd_data_frame=df_data, x_name="a", y_name="c", return_intermediate=True
    )
