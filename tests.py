...
class TestClass(object):
    ...
    def test_normal(self):
        self.data_1 = {
            ...
            'somedate': [datetime.date(2011, 7, 4), datetime.datetime(2022, 1, 1, 13, 57),
                         datetime.datetime(1990, 12, 9), np.nan, datetime.datetime(1990, 12, 9),
                         datetime.datetime(1950, 12, 9), datetime.datetime(1898, 1, 2), datetime.datetime(1950, 12, 9),
        ...
        # threshold = 0.4
        self.df_1_expected = {'bool_01': {0: 4, 1: 5}, 'bool_01_with_nan': {0.0: 4, 1.0: 4},
                              'bool_tf': {False: 3, True: 6}, 'bool_tf_with_nan': {False: 5, True: 3}, 's1': {1.0: 9},
                              'x': np.array([1, 3, 1, 0, 1, 0, 0, 0, 0, 2], dtype=np.int64),
                              'y': np.array([6, 0, 1, 0, 0, 0, 0, 0, 0, 1], dtype=np.int64)}
    ...
    def test_corner(self):
        self.df_2 = pd.DataFrame(
            {'all_nan': [np.nan for _ in range(10)], 'all_one': np.ones(10), 'all_zeros': np.zeros(10),
             'random': np.array([0.38538395, 0.13609054, 0.15973238, 0.96192966, 0.03708882,
